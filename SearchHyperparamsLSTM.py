...
def build_lstm_model(hp, X_train):
    ...
    for i in range(num_layers):
        ...
        lstm_model.add(tf.keras.layers.LSTM(units=units, activation=activation, return_sequences=True if i < num_layers - 1 else False, input_shape=(X_train.shape[1], 1)))
...
if __name__ == "__main__":
    ...
    tuner = kt.RandomSearch(
        hypermodel=lambda hp: build_lstm_model(hp, X_train),
        objective="val_mean_absolute_error",
        max_trials=1,
        executions_per_trial=2,
        overwrite=True,
        directory="hyperparams_lstm",
        project_name="для_отчета_1_lstm"
    )
