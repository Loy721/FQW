...
def build_model(hp):
    ...
    mlp_model.add(tf.keras.layers.Dense(
        ...
        activation=hp.Choice("activation_l1", ["relu", "tanh"]),
        input_dim=50))
    # mlp_model.add(tf.keras.layers.Dense(
    #     units=hp.Int("units_l2", min_value=16, max_value=1024, step=16),
    #     activation=hp.Choice("activation_l2", ["relu", "tanh"])))
    # mlp_model.add(tf.keras.layers.Dense(
    #     units=hp.Int("units_l3", min_value=16, max_value=1024, step=16),
    #     activation=hp.Choice("activation_l3", ["relu", "tanh"])))
